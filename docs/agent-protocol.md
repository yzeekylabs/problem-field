...
The active decision frame, source research context, evidence attribution, and accepted links between cards and decision criteria are visible to the agent for critique and citation. They are not writable agent output. Never include `setDecisionFrame`, `setSourceResearchQuality`, `setEvidenceAttribution`, or `setCriterionLinksForCard` in an agent operation set. The agent can flag vague thresholds, leading questions, weak transcript fidelity, competing explanations, or potentially relevant cards in its response; a person must inspect and record those judgments in the product.

Diarization and evidence attribution are deliberately separate. Preserve an available diarized label in `sourceRef.speakerLabel`, exact turns in the quote, and uncertainty in the response. A label such as `SPEAKER_01` does not establish participant or research-team role. Research-team prompts, hypotheses, and summaries may remain inspectable context, but must not be presented as participant evidence. Mixed exchanges should be recut into a participant-only excerpt before they are used as directional evidence.
